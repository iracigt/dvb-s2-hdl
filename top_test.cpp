...
// const int MAX_CLKS = 256 * 2 * 16 * (FRAME_SIZE_NORMAL / 3 + 90);
// onst int MAX_CLKS = 2 * 16 * 200;
// const int MAX_CLKS = 2 * 16 * (FRAME_SIZE_NORMAL / 3 + 90);
const int MAX_CLKS = 1024 * 2 * 16 * (FRAME_SIZE_NORMAL / 3 + 90);
// const int MAX_CLKS = 2 * 16 * (10+ 90);
...
int main(int argc, char** argv, char** env) {
    ...
    for (int i = 0; i < MAX_CLKS; i++) {
        if (tb.is_ready()) {
            ...
            uint8_t x = 0;
            if (fread(&x, sizeof(char), 1, infile) != 1) break;
            tb.set_data(x);
            //tb.set_data(0x00);
